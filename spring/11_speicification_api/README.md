...
Подобные запросы можно делать на разных инстументах. Из них при работе с JPA обычно выделяют 3:
...
на Criteria API упрощенный способ работы с этим самым Criteria API.
...
Мы сегодня будет говорить о Specification API.
...
Обратите внимание на последний метод. Спецификации можно соединять между собой через методы ```.and()``` или ```.or()```.
Это позволяет элементарным образом учитывать глобальные требования вашей разрабатываемой системы, касающиеся доступа к данным.
...
```LazyInitializationException``` при попытке обратиться к пропертям класса ```Department``` внутри ```Employee``` 
(```emp.getDepartment().getName()```, например). Т.е. запрос с join делает этот самый join, но не парсит весь его результат 
в нужные сущности, а лишь собирает ```root``` сущность - ```Employee```. Соответственно запросы с join можно использовать 
для повышения производительности, когда дочерние ленивые сущности в рамках запроса нам не нужны.
...
```    public static Specification<Employee> ofEmployeeFilter(EmployeeFilter employeeFilter)``` внутри  ```EmployeeSpecifications```,
 ...
 Бонусные очки (необязательно задание) будут выданы падавану, который сделает запрос по депаратментам регистро-независимым и 
 напишет соответствующие тесты-проверки.
