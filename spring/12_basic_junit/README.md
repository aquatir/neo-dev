...
Note: по каким-то причинам даже последняя идея пишет, что ```could not autowire. No beans of 'JacksonTester` found``` 
на поле с ```JacksonTester```. Это скорее всего неправда, т.к. этот класс конфигается автоматически при указании аннотации 
```JsonTest```. Самый простой вариант - запустить тест, не смотря на наличие этого сообщения.
