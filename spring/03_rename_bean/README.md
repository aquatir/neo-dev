...
- !!! ```@Transactional``` НЕ БУДЕТ РАБОТАТЬ, если он стоит над ```protected``` методом (```protected``` 
метод хоть и попадает в прокси-класс-наслед, но с ним может работать только CGLIB, а не Dynamic JDK Proxy, 
т.к. в интерфейсах не может быть protected методов. Поэтому командой спринга принято решение НЕ ПОДДЕРЖИВАТЬ 
protected методы вообще) 
...
Если не хочется писать все эти констукторы самому - можно воспользоваться замечательной библиотечкой [Lombok[4]](https://projectlombok.org/). Тогда код получится совсем простой
```
@Service 
