...
все еще будет осуществляться фрейморком. Таким образом, JDBC Template - это легковестная обвязка вокруг прямого доступа к
возможностям JDBC драйвера, но на стероидах, за счет автоконфигурации всего, что можно сконфигурировать. В том числе и транзакции 
через ```@Transactional```.
...
Затем завайрить один из классов в свой Data Access Object (DAO) объект, например:
```
@Component
...
Терминология с ```DTO``` и ```Repository``` не является общепринятой, а является исключительно выдумкой автора :)
...
Note: JDBC Template практически не делает магии. Так как update запрос в обычном SQL возвращает количество измененынх 
рядом - в JDBC Template он делает тоже самое. Ровно как и insert. Ровно как и delete. Это одна из причин, почему delete
...
либо через упрощенный (на самом деле не особо упрощенный) builder-класс-помощник.
```SimpleJdbcInsert:
